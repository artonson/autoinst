...
class TarlExtractor:
    ...
    def extract_instance(self, pts, name):
        ...
        extracted_features = torch.zeros(
            (pts.shape[0], 1))  # to save indices accordingly
        extracted_features[mapping_idcs] = 1
        non_mapped_idcs = torch.nonzero(
            extracted_features == 0).squeeze().reshape(-1, 2)[:, 0]
        ...
        distances = torch.cdist(torch.from_numpy(pts[non_mapped_idcs, :3]).cuda(),
                                torch.from_numpy(pts[mapping_idcs, :3]).cuda())  # speedup by moving to GPU
    ...
    def point_set_to_coord_feats(self, point_set, deterministic=False):
        ...
        _, mapping = ME.utils.sparse_quantize(
            coordinates=p_coord, return_index=True)
        ...
        for lidar_name in tqdm(all_lidars):
            if self.dataset == 'nuscenes':
                points = np.fromfile(os.path.join(
                    self.input_dir, lidar_name), dtype=np.float32).reshape(-1, 5)[:, :4]
            elif self.dataset == 'kitti':
                points = np.fromfile(os.path.join(
                    self.input_dir, lidar_name), dtype=np.float32).reshape(-1, 4)
            else:
