...
class PseudoSemanticDatasetOversegmentedModule(LightningDataModule):
    ...
    def setup(self, stage=None):
        train_set = PseudoSemanticOversegmentedDataset(
            self.cfg[self.cfg.MODEL.DATASET].PATH + "/sequences/",
            self.cfg[self.cfg.MODEL.DATASET].CONFIG,
            split="train",
            dataset=self.dataset,
        )
        self.train_mask_set = PseudoMaskSemanticOversegmentedDataset(
            dataset=train_set,
            split="train",
        ...
        val_set = PseudoSemanticOversegmentedDataset(
            self.cfg[self.cfg.MODEL.DATASET].PATH + "/sequences/",
            self.cfg[self.cfg.MODEL.DATASET].CONFIG,
            split="valid",
            dataset=self.dataset,
        )
        self.val_mask_set = PseudoMaskSemanticOversegmentedDataset(
            dataset=val_set,
            split="valid",
            min_pts=self.cfg[self.cfg.MODEL.DATASET].MIN_POINTS,
            space=self.cfg[self.cfg.MODEL.DATASET].SPACE,
        ...
        test_set = PseudoSemanticOversegmentedDataset(
            self.cfg[self.cfg.MODEL.DATASET].PATH + "/sequences/",
            self.cfg[self.cfg.MODEL.DATASET].CONFIG,
            split="test",
            dataset=self.dataset,
        )
        self.test_mask_set = PseudoMaskSemanticOversegmentedDataset(
            dataset=test_set,
            split="test",
            min_pts=self.cfg[self.cfg.MODEL.DATASET].MIN_POINTS,
            space=self.cfg[self.cfg.MODEL.DATASET].SPACE,
...
class PseudoSemanticOversegmentedDataset(Dataset):
    def __init__(self, data_path, cfg_path, split="train", dataset="KITTI"):
        ...
        #data_path = '/mnt/hdd/Datasets/oversegmented_labels/'
        data_path = '/mnt/hdd/Datasets/oversegmented_test/'
        self.im_idx = []
        self.split = split
    ...
    def __getitem__(self, index):
        ...
        data = np.load(self.im_idx[index], allow_pickle=True).reshape((-1, 5))
        xyz = data[:, :3]
        labels =  data[:,4].astype(int) ## currently no intensity information is used 
        intensity = data[:, 3]
        ...
        if self.split == "test":
            annotated_data = np.expand_dims(
                np.zeros_like(xyz[:, 0], dtype=int), axis=1
            )
            ins_labels = labels + 1
            ...
            ins_labels = labels + 1
            sem_labels = np.zeros_like(labels).reshape(-1,1) 
        return (xyz, sem_labels, ins_labels, intensity,fname, token)


class PseudoMaskSemanticOversegmentedDataset(Dataset):
    def __init__(
        self,
        dataset,
        split,
    ...
    def __getitem__(self, index):
        ...
        while empty == True:
            ...
            xyz, sem_labels, ins_labels, intensity, fname,  token = data
            keep = np.argwhere(
                (self.xlim[0] < xyz[:, 0])
                & (xyz[:, 0] < self.xlim[1])
                & (self.ylim[0] < xyz[:, 1])
        ...
        #print(ins_labels.shape,sem_labels.shape)
        
        #print('ins labels',np.unique(_ins_labels))
        ...
        ## just set to go through the pass filter
        #print('sem labels',np.unique(sem_labels)) 
        #print(self.th_ids)
        #keep_th = np.array(
