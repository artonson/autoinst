...
std::vector<PointAPR> CVC::calculateAPR(Eigen::MatrixXf cloud_IN){
        ...
        for (int i =0; i<cloud_IN.rows(); ++i){
           ...
           par.azimuth =(float) atan2(cloud_IN(i,2), par.range);
...
std::unordered_map<int, Voxel> CVC::build_hash_table(const std::vector<PointAPR>& vapr){
...
std::vector<int> CVC::most_frequent_value(std::vector<int> values) {
