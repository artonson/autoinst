...
def aggregate_pointcloud(
    dataset, ind_start, ind_end, icp=False, icp_threshold=0.9, ground_segmentation=None
):
    np.random.seed(42)
    random.seed(42)
    """
    ...
    if ground_segmentation is None:
        ...
        for i in range(ind_start, ind_end):
            ...
            if icp and i != ind_start:
                ...
                reg_p2l = registration.registration_icp(
                    pcd,
                    map_pcd,
                    icp_threshold,
                    transform,
                    registration.TransformationEstimationPointToPlane(),
                    registration.ICPConvergenceCriteria(max_iteration=1000),
                )
        ...
        for i in tqdm(range(ind_start, ind_end)):
            ...
            if ground_segmentation == "patchwork":
                ...
                _, ground_idcs = pcd.segment_plane(
                    distance_threshold=0.4, ransac_n=3, num_iterations=2000
                )
                nonground_idcs = np.setdiff1d(
                    np.arange(0, np.asarray(pcd.points).shape[0]), ground_idcs
                )
            else:
                raise ValueError(
                    'ground_segmentation must be either "None", "patchwork" or "open3d"'
                )
            ...
            if icp:
                ...
                pcd_nonground.estimate_normals(
                    search_param=o3d.geometry.KDTreeSearchParamHybrid(
                        radius=0.5, max_nn=200
                    )
                )
