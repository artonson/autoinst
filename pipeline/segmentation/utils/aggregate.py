...
lib_path = os.path.expanduser(
    '~') + '/unsup_3d_instances/pipeline/segmentation/utils/voxel_clustering_dependencies/build/'
...
class AggregationClustering():
    ...
    def overlap_clusters(self, cluster_i, cluster_j, min_cluster_point=10):
        # get unique labels from pcd_i and pcd_j from segments bigger than
        # min_clsuter_point
    ...
    def aggregate_pcds(self, data_batch, data_dir, use_ground_label=True):
        ...
        poses = self.load_poses_kitti(
            os.path.join(
                datapath,
                'calib.txt'),
            os.path.join(
                datapath.split('sequences')[0] +
                'poses',
                self.sequence +
                '.txt'))
        ...
        for t in range(len(data_batch)):
            ...
            p_set[:, :3] = self.apply_transform(
                p_set_orig[:, :3], poses[pose_idx])
        ...
        pcd_parse_idx = np.unique(
            np.argwhere(
                ground_label == g_delimiter)[
                :, 0])

        pose_idx = int(fname)
        points_set[:, :3] = self.undo_transform(
            points_set[:, :3], poses[pose_idx])
    ...
    def aggregate_pcds_nuscenes(self, data_batch):
        ...
        for t in range(len(data_batch)):
            p_set = self.dataset.get_point_cloud(
                data_batch[t], pose_correction=True)
            p_set_orig = self.dataset.get_point_cloud(
                data_batch[t], pose_correction=False)
            # aggregate a delimiter and the next scan
            points_set = np.vstack([points_set, p_delimiter, p_set])
            labels = np.ones((p_set.shape[0], 1))
    ...
    def clusterize_pcd(self, points, ground):
        ...
        labels_ = np.expand_dims(
            self.clusters_hdbscan(
                np.asarray(
                    pcd_.points)),
            axis=-1)
    ...
    def visualize_pcd_clusters(self, points, labels):
        ...
        colors_instance = plt.get_cmap("prism")(np.arange(
            len(flat_indices)) / (max_instance if max_instance > 0 else 1))
