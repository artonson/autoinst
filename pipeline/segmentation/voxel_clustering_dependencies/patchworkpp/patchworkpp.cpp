        ...
        for (int ring_idx = 0; ring_idx < params_.num_rings_each_zone[zone_idx]; ++ring_idx) {
            for (int sector_idx = 0; sector_idx < params_.num_sectors_each_zone[zone_idx]; ++sector_idx) {
                ...
                */
                ...
                for (auto &p : regionwise_nonground_) {
                        int idx = point_dict[p];
                        nonground_idcs.push_back(idx);
                    }
...
std::vector<int> PatchWorkpp::get_ground_idcs(){
    return ground_idcs;
}

std::vector<int> PatchWorkpp::get_nonground_idcs(){
    return nonground_idcs;
}
