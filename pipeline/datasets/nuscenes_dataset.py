...
class NuScenesDataset(AbstractDataset):
    ...
    def get_files(self):
        ...
        while next_sample != '':
            ...
            self.tokens['CAM_FRONT_LEFT'].append(
                sample['data']['CAM_FRONT_LEFT'])
            self.tokens['CAM_FRONT_RIGHT'].append(
    ...
    def get_lidar_pose(self, index):
        ...
        egopose_data = self.dataset.get(
            'ego_pose', lidar_data["ego_pose_token"])

        pose_car = geoutils.transform_matrix(
            egopose_data["translation"], Quaternion(
                egopose_data['rotation']))
    ...
    def get_lidar_calib(self, index):
        ...
        car_to_velo = geoutils.transform_matrix(
            calib_data["translation"], Quaternion(
                calib_data['rotation']))
        return car_to_velo

    def get_panoptic_labels(self, index):
        # requires getting point cloud as well to find the indeces to remove
        # from range filter
        lidar_token = self.tokens['LIDAR_TOP'][index]
        lidar_data = self.dataset.get('sample_data', lidar_token)
    ...
    def get_image(self, index):
        ...
        image_data_front_left = self.dataset.get(
            'sample_data', cam_token_front_left)
        image_data_front_right = self.dataset.get(
            'sample_data', cam_token_front_right)

        # open jpg.
        image_front = Image.open(
            os.path.join(
                self.dataset_path,
                image_data_front["filename"]))
        image_front_left = Image.open(
            os.path.join(
                self.dataset_path,
                image_data_front_left["filename"]))  # open jpg.
        image_front_right = Image.open(
            os.path.join(
                self.dataset_path,
                image_data_front_right["filename"]))  # open jpg.
        return {
            'CAM_FRONT': image_front,
            'CAM_FRONT_LEFT': image_front_left,
            'CAM_FRONT_RIGHT': image_front_right}
    ...
    def get_calibration_matrices(self, index):
        ...
        calib_data_front = self.dataset.get(
            "calibrated_sensor",
            image_data_front["calibrated_sensor_token"])
        calib_data_front_left = self.dataset.get(
            "calibrated_sensor", image_data_front_left["calibrated_sensor_token"])
        calib_data_front_right = self.dataset.get(
            "calibrated_sensor", image_data_front_right["calibrated_sensor_token"])
        return {'CAM_FRONT': calib_data_front,
                'CAM_FRONT_LEFT': calib_data_front_left,
                'CAM_FRONT_RIGHT': calib_data_front_right}
